...
#if !KIERO_USE_MINHOOK
static std::map<uint16_t, std::pair<void**, void*>> g_activeHooks;
#endif
...
#if KIERO_USE_MINHOOK
				// MH_Initialize(); not needed, we use detours
#else
#endif
...
void kiero::shutdown()
{
	...
	{
#if KIERO_USE_MINHOOK
		// MH_DisableHook(MH_ALL_HOOKS); not needed, we use detours >:(
#else
		if (!g_activeHooks.empty()) {
			DetourTransactionBegin();
			DetourUpdateThread(GetCurrentThread());

			for (auto const& entry : g_activeHooks) {
				auto hookRecord = &entry.second;
				DetourDetach(hookRecord->first, hookRecord->second);
			}
...
kiero::Status::Enum kiero::bind(uint16_t _index, void** _original, void* _function)
{
	...
	{
#if KIERO_USE_MINHOOK
		// void* target = (void*)g_methodsTable[_index]; FUCK MINHOOK
		...
		if (target == NULL) return Status::UnknownError;
		...
		*_original = target;
		...
		if (DetourAttach((PVOID*)_original, _function) != NO_ERROR) {
			DetourTransactionAbort();
			return Status::UnknownError;
		}

		if (DetourTransactionCommit() == NO_ERROR) {
			g_activeHooks[_index] = { _original, _function };
		} else {
			return Status::UnknownError;
		}
#endif
...
void kiero::unbind(uint16_t _index)
{
	...
	{
#if KIERO_USE_MINHOOK
		// MH_DisableHook((void*)g_methodsTable[_index]); fuck minhook
#else
		auto it = g_activeHooks.find(_index);
		if (it != g_activeHooks.end()) {
			void** ppReal = it->second.first;
			void* pDetour = it->second.second;

			DetourTransactionBegin();
			DetourUpdateThread(GetCurrentThread());

			if (DetourDetach(ppReal, pDetour) == NO_ERROR) {
